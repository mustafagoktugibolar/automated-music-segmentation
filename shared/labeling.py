...
def _segment_durations(segments: list[dict]) -> np.ndarray:
    return np.asarray(
        [max(0.0, float(s.get("end", 0.0) or 0.0) - float(s.get("start", 0.0) or 0.0)) for s in segments],
        dtype=float,
    )


def build_segment_descriptors(file_path: str, segments: list[dict]) -> np.ndarray | None:
    """Build deterministic lightweight descriptors for segment-level clustering.

    This is intentionally optional. Backend-only callers can use fallback labels
    without importing librosa/sklearn, while worker processes can pass audio.
    """
    if not file_path or not segments:
        return None
    try:
        import librosa
        ...
        for seg in segments:
            ...
            onset_density = float(np.mean(onset_block > np.percentile(onset, 75))) if onset_block.size else 0.0

            descriptors.append(
                np.concatenate(
                    [
                        np.mean(chroma_block, axis=1),
                        np.std(chroma_block, axis=1),
                        np.mean(mfcc_block, axis=1),
                        np.std(mfcc_block, axis=1),
                        [float(np.mean(rms_block)) if rms_block.size else 0.0],
                        [float(np.std(rms_block)) if rms_block.size else 0.0],
                        [onset_density],
                        [seg_dur / max(duration, 1.0)],
                    ]
                ).astype(np.float32)
            )
...
def _cluster_descriptors(descriptors: np.ndarray) -> tuple[np.ndarray, float, str] | None:
    ...
    # Dependency-free deterministic fallback: assign a new cluster when a
    # descriptor is far from all previous centroids. This preserves repeated
    # nearby sections without fabricating semantic names.
    labels: list[int] = []
...
def _energy_profile(segments: list[dict], descriptors: np.ndarray | None = None) -> np.ndarray:
    if descriptors is not None and len(descriptors) == len(segments) and descriptors.shape[1] >= 3:
        # The descriptor builder appends RMS mean near the end; using the row
        # norm is more robust for externally supplied descriptors.
        values = np.linalg.norm(np.nan_to_num(descriptors), axis=1)
    else:
        values = np.asarray([float(s.get("confidence", 0.5) or 0.5) for s in segments], dtype=float)
    if values.size == 0:
        return values
...
def assign_semantic_labels(
    ...
    # Identify which segments are silence so Intro/Outro can skip past them.
    ...
    for idx, seg in enumerate(out):
        ...
        if _silence_flags[idx]:
            ...
            # First non-silence section in opening 20% → Intro.
            semantic = "Intro"
            ...
            # Last non-silence section in closing 25% → Outro.
            semantic = "Outro"
            confidence = 0.65 if counts[structural] == 1 else 0.52
        ...
        # Middle segments with no matched pattern: use position for a basic label.
...
def apply_two_layer_labels(
    ...
    *,
    ...
    # Build descriptors once so both stages share the same audio features.
    # Previously assign_structural_labels built them internally but didn't
    # return them, so assign_semantic_labels got no descriptors and the
    # energy-based Chorus/Verse detection always fell back to confidence
    # values (all ~0.5), making chorus_label always None.
    if descriptors is None and file_path:
        descriptors = build_segment_descriptors(file_path, segments)
    labeled = assign_structural_labels(
        segments,
        descriptors=descriptors,
        file_path=None,  # already loaded above
