...
from segmentation.infrastructure.audio.decoder import FFMPEG_BIN, load_audio
from segmentation.infrastructure.audio.features import build_segment_descriptors_from_audio
...
from segmentation.core.labeling.ml_sequence import predict_semantic_labels_sequence
from segmentation.infrastructure.logging import get_logger
from segmentation.application.segmentation.multi_feature_fusion import (
...
# ---------------------------------------------------------------------------
# Stage 0 — Audio loading (delegated to segmentation.infrastructure.audio.decoder)
# ---------------------------------------------------------------------------

logger.info("ffmpeg binary: %s", FFMPEG_BIN or "NOT FOUND — falling back to audioread (slow)")


def _load_audio_from_bytes(content: bytes, sr: int = _SR) -> tuple[np.ndarray, int]:
    """Load audio from in-memory bytes via segmentation.infrastructure.audio.decoder (ffmpeg preferred)."""
    return load_audio(content, sr=sr)
...
def _analyze_content(
    ...
    from segmentation.infrastructure.config import AppSettings as _AppSettings
    labeling_method = str(p.get("labeling_method", _AppSettings.LABELING_METHOD))
    ...
    if _file_path is not None:
        y, sr = load_audio(_file_path, sr=_SR)
    else:
        y, sr = load_audio(content, sr=_SR)
