...
from segmentation.api.routes.schemas import SegmentationParams, StorageSegmentationRequest
...
from segmentation.application.orchestration.result_listener import register_sse_callback, unregister_sse_callback
...
async def upload_and_segment_audio(
    ...
    try:
        params_dict = parsed_params.model_dump(exclude_none=True) if parsed_params else None
        task_id = await orchestrator.process_upload(file, requested_algos, params_dict, webhook_url)
        return {
...
async def segment_from_storage(req: StorageSegmentationRequest):
    try:
        params_dict = req.params.model_dump(exclude_none=True) if req.params else None
        task_id = await orchestrator.process_from_storage(req.song_id, req.algorithms, params_dict)
        return {
