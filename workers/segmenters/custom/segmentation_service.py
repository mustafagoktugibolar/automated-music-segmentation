...
# ---------------------------------------------------------------------------
# Stage 0 — Audio loading (delegated to shared.audio_io)
# ---------------------------------------------------------------------------
...
def _load_audio_from_bytes(content: bytes, sr: int = _SR) -> tuple[np.ndarray, int]:
    """Load audio from in-memory bytes via shared.audio_io (ffmpeg preferred)."""
...
def _analyze_content(
    ...
    from shared.config import AppSettings as _AppSettings
